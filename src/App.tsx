...
import { Users, Trophy, Eye, EyeOff, Tag, RotateCcw, Info, ArrowRight, Check, ShieldCheck, X, Moon, Sun } from 'lucide-react';
...
export default function App() {
  ...
  return (
    <div className={`min-h-[100dvh] transition-colors duration-300 ${isDarkMode ? 'bg-[#121212] text-white' : 'bg-[#FFFDF5] text-black'} font-serif overflow-hidden relative selection:bg-[var(--color-primary)] selection:text-white flex flex-col justify-between`}>
      ...
      <header className={`w-full z-20 p-4 flex justify-between items-center sm:px-8 border-b-2 border-black/10 transition-colors duration-300 ${isDarkMode ? 'bg-[#121212]/80 border-white/10' : 'bg-[#FFFDF5]/80'} backdrop-blur-md`}>
        ...
        <div className="flex items-center gap-4">
          <div className={`flex items-center gap-2 text-sm font-sans font-bold px-3 py-1 rounded-full border-2 transition-colors duration-300 ${isDarkMode ? 'text-emerald-400 bg-emerald-900/40 border-emerald-400' : 'text-emerald-700 bg-emerald-100 border-emerald-700'}`}>
      ...
      <main className="w-full max-w-md mx-auto z-10 p-6 sm:p-8 rounded-2xl relative flex-1 flex flex-col justify-center">
        <AnimatePresence mode="wait">
          ...
          {phase === 'setup' && (
            ...
            >
              ...
              <form onSubmit={handleStartGame} className="flex flex-col gap-4 mt-6">
                <div className="flex flex-col gap-2">
                  ...
                  <input
                    ...
                    className={`w-full text-xl p-4 brutal-border focus:outline-none focus:ring-4 focus:ring-[#FF6321]/50 transition-colors duration-300 ${isDarkMode ? 'bg-zinc-900 text-white placeholder-zinc-500' : 'bg-white text-black placeholder-gray-400'}`}
                  />
                </div>
                <button
                  type="submit"
                  disabled={!category.trim()}
                  className={`w-full brutal-border text-xl py-4 mt-2 disabled:opacity-50 transition-all font-sans font-black flex justify-center items-center gap-2 ${isDarkMode ? 'bg-white text-black hover:bg-gray-200' : 'bg-black text-white hover:bg-black/90'}`}
                >
              ...
              <button
                ...
                className={`text-sm font-sans font-bold underline flex items-center justify-center gap-1 mx-auto mt-4 transition-colors ${isDarkMode ? 'text-gray-300 hover:text-white' : 'text-gray-700 hover:text-black'}`}
              >
              ...
              <AnimatePresence>
                {showHowToPlay && (
                  ...
                  >
                    <div className={`brutal-border p-4 text-sm mt-2 rounded transition-colors duration-300 ${isDarkMode ? 'bg-zinc-800 text-gray-200' : 'bg-gray-100 text-black'}`}>
          ...
          {phase === 'passPhone' && (
            ...
            >
              ...
              <div className="space-y-2">
                ...
                <div className="text-5xl font-black brutal-border bg-yellow-300 text-black inline-block px-6 py-2 transform rotate-2">
              ...
              <button
                onClick={handleReady}
                className={`w-full mt-8 brutal-border text-xl py-5 font-sans font-black active:scale-95 transition-transform ${isDarkMode ? 'bg-white text-black hover:bg-gray-200' : 'bg-black text-white hover:bg-gray-900'}`}
              >
              ...
              {entries.length >= 2 && (
                <button
                  ...
                  className={`mt-4 brutal-border text-lg py-3 px-6 font-sans font-bold flex items-center gap-2 justify-center transition-colors ${isDarkMode ? 'bg-zinc-800 text-white hover:bg-zinc-700' : 'bg-white text-black hover:bg-gray-100'}`}
                >
              ...
              <form onSubmit={handleNextPlayer} className="flex flex-col gap-6 mt-4 py-4">
                <div className="flex flex-col gap-3">
                  <label className="font-sans font-black text-xl text-center flex flex-col items-center gap-2">
                    Enter your word:
                    <div className={`text-xs font-serif font-normal px-2 py-1 rounded border flex items-center gap-1 w-fit transition-colors duration-300 ${isDarkMode ? 'text-emerald-400 bg-emerald-900/40 border-emerald-400' : 'text-emerald-700 bg-emerald-100 border-emerald-700'}`}>
                  ...
                  <div className="relative">
                    <input
                      ...
                      className={`w-full text-2xl p-4 brutal-border pr-14 text-center focus:outline-none focus:ring-4 focus:ring-black/20 ${isDarkMode ? 'bg-zinc-900 text-white placeholder-zinc-500' : 'bg-white text-black placeholder-gray-400'}`}
                    />
                    <button
                      type="button"
                ...
                <p className={`text-center text-sm font-bold mb-4 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
          ...
          {phase === 'reveal' && (
            ...
            >
              <div className={`text-center pb-4 border-b-4 ${isDarkMode ? 'border-white' : 'border-black'}`}>
                <h2 className="text-3xl font-sans font-black uppercase tracking-tight text-[#FF6321]">
                  The List
                </h2>
                <div className={`inline-block mt-2 font-bold px-3 py-1 brutal-border text-sm ${isDarkMode ? 'bg-white text-black' : 'bg-black text-white'}`}>
                  Category: {category}
                </div>
              </div>
              ...
              <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 pb-4 space-y-3">
                {shuffledWords.map((word, index) => (
                  <motion.div
                    ...
                    key={index}
                    className={`flex brutal-border text-xl font-bold ${isDarkMode ? 'bg-zinc-900 text-white' : 'bg-white text-black'}`}
                  >
                    <div className={`bg-yellow-300 border-r-4 w-12 flex items-center justify-center font-sans font-black text-2xl shrink-0 text-black ${isDarkMode ? 'border-white' : 'border-black'}`}>
                      {index + 1}
                    </div>
              ...
              <button
                ...
                className={`w-full brutal-border text-xl py-4 font-sans font-black flex items-center justify-center gap-2 mt-auto transition-colors ${isDarkMode ? 'bg-white text-black hover:bg-gray-200' : 'bg-black text-white hover:bg-gray-900'}`}
              >
        ...
        {(showTerms || showPrivacy) && (
          ...
          >
            <motion.div
              ...
              animate="visible"
              exit="exit"
              className={`max-w-lg w-full max-h-[80vh] brutal-border flex flex-col rounded-xl overflow-hidden ${isDarkMode ? 'bg-zinc-900 text-white' : 'bg-[#FFFDF5] text-black'}`}
            >
              ...
              <div className={`p-6 overflow-y-auto custom-scrollbar font-serif leading-relaxed text-sm space-y-4 ${isDarkMode ? 'text-gray-300' : 'text-gray-800'}`}>
                {showPrivacy ? (
                  <>
                    ...
                    <h3 className={`font-sans font-bold text-lg ${isDarkMode ? 'text-white' : 'text-black'}`}>2. Data Storage</h3>
                    ...
                    <h3 className={`font-sans font-bold text-lg ${isDarkMode ? 'text-white' : 'text-black'}`}>3. Analytics & Tracking</h3>
                  ...
                  <>
                    <h3 className={`font-sans font-bold text-lg ${isDarkMode ? 'text-white' : 'text-black'}`}>1. Acceptance of Terms</h3>
                    <p>By accessing and using this game, you accept and agree to be bound by the terms and provision of this agreement.</p>

                    <h3 className={`font-sans font-bold text-lg ${isDarkMode ? 'text-white' : 'text-black'}`}>2. Responsible Use</h3>
                    ...
                    <h3 className={`font-sans font-bold text-lg ${isDarkMode ? 'text-white' : 'text-black'}`}>3. Disclaimer of Warranties</h3>
